...
export default function CandidateTable({ candidates, onUpdate }: Props) {
  ...
  const candidatosFiltrados = candidates.filter((c: Candidate) => {
    ...
    return c.nombre.toLowerCase().includes(q) || c.dni?.includes(q) || c.puesto?.toLowerCase().includes(q) || (c.areas_exp || []).join(' ').toLowerCase().includes(q);
  });

  const actualizarObservacion = async (id: string, texto: string) => {
  ...
  const exportCSV = () => {
    const headers = ["FECHA", "NOMBRE", "DNI", "NAC.", "TELÉFONO", "EMAIL", "LOC", "PROV", "LINKEDIN", "PUESTO", "CONDUCTA", "CARNET", "EDU.", "TITULO", "AREAS", "HABILIDADES", "EXP", "CARGO", "EMPRESA", "TESTS", "OBS. ADMIN"];
    let csv = "\uFEFF" + headers.join(";") + "\r\n";
    candidatosFiltrados.forEach((c: Candidate) => {
      const row = [`"${c.fecha}"`, `"${c.nombre}"`, `"${c.dni || ''}"`, `"${c.fecha_nac || ''}"`, `"${c.telefono || ''}"`, `"${c.email || ''}"`, `"${c.localidad || ''}"`, `"${c.provincia || ''}"`, `"${c.linkedin || ''}"`, `"${c.puesto || ''}"`, `"${c.buena_conducta || ''}"`, `"${c.carnet_manejo || ''}"`, `"${c.nivel_educativo || ''}"`, `"${c.titulo || ''}"`, `"${(c.areas_exp || []).join(', ')} ${c.areas_exp_otros ? '(' + c.areas_exp_otros + ')' : ''}"`, `"${(c.habilidades || []).join(', ')} ${c.habilidades_otros ? '(' + c.habilidades_otros + ')' : ''}"`, `"${c.anios_exp || ''}"`, `"${c.ultimo_cargo || ''}"`, `"${c.ultima_empresa || ''}"`, `"${(c.test_results || []).map((t: any) => t.test + ': ' + t.score).join(' | ')}"`, `"${c.observaciones || ''}"`];
      csv += row.join(";") + "\r\n";
    });
    ...
    <>
      <section className="bg-[#1A1A1A] overflow-hidden">
        <div className="p-4 sm:p-6 flex flex-wrap justify-between items-center border-b border-[#2A2A2A] gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center w-full sm:w-auto">
            ...
            <button onClick={exportCSV} className="flex items-center justify-center gap-2 bg-[#1F1F1F] text-[#E6CA65] border border-[#E6CA65]/40 hover:bg-[#2A2A2A] px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap">📊 Exportar Excel</button>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-[#2A2A2A] text-sm text-gray-300">
              {candidatosFiltrados.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-8 text-gray-500">No hay postulaciones.</td></tr>
                ...
                <tr key={c.id} className="hover:bg-[#222]">
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="flex items-center justify-center gap-2 flex-wrap">
                      <button onClick={() => setPerfilParaVer(c)} className="text-xs text-blue-400 bg-blue-900/20 hover:bg-blue-900/40 px-2 py-1 rounded flex items-center gap-1 mb-1">👁️ Perfil</button>
                      {c.cv_file_url && (<button onClick={() => setCvParaVer(c)} className="text-xs text-gray-200 bg-[#2B2B2B] hover:bg-[#383838] px-2 py-1 rounded border border-[#444] flex items-center gap-1 mb-1"><span className="text-[#E6CA65]">📄</span> CV</button>)}
                      <button onClick={() => eliminarCandidato(c.id)} className="text-xs text-red-400 bg-red-900/20 hover:bg-red-900/40 px-2 py-1 rounded flex items-center gap-1 mb-1">🗑️ Borrar</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {cvParaVer && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A1A] border border-[#333] rounded-xl w-full max-w-4xl h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-[#2A2A2A] flex justify-between items-center bg-[#1F1F1F]">
              <div className="flex items-center gap-3">
                <span className="text-xl text-[#E6CA65]">📄</span>
                <div><h4 className="text-sm font-semibold text-white">{cvParaVer.cv_file_name}</h4><p className="text-xs text-gray-400">Candidato: {cvParaVer.nombre}</p></div>
              </div>
              <div className="flex items-center gap-3">
                <a href={cvParaVer.cv_file_url} download={cvParaVer.cv_file_name} className="inline-flex items-center gap-1.5 text-xs text-black bg-[#E6CA65] hover:bg-[#d8bd58] px-3 py-1.5 rounded-lg font-bold">⬇️ Descargar</a>
                <button onClick={() => setCvParaVer(null)} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-[#2F2F2F]">✖️</button>
              </div>
            </div>
            <div className="flex-1 bg-[#121212] p-2"><iframe src={cvParaVer.cv_file_url} title={`CV de ${cvParaVer.nombre}`} className="w-full h-full rounded border border-[#222]" /></div>
          </div>
        </div>
      )}
      ...
      {perfilParaVer && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A1A] border border-[#333] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto scrollbar-hide shadow-2xl">
            <div className="p-4 sm:p-6 border-b border-[#2A2A2A] flex justify-between items-center sticky top-0 bg-[#1F1F1F] z-10">
              <h3 className="text-lg font-bold text-[#E6CA65]">Perfil Completo de {perfilParaVer.nombre}</h3>
              <button onClick={() => setPerfilParaVer(null)} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-[#2F2F2F]">✖️</button>
            </div>
            <div className="p-4 sm:p-6 space-y-6 text-sm">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div><p className="text-gray-500 text-xs">CV Adjunto</p>{perfilParaVer.cv_file_url ? <button onClick={() => setCvParaVer(perfilParaVer)} className="text-blue-400 hover:underline text-xs flex items-center gap-1 mt-1">📄 Ver/Descargar</button> : <p className="text-white">No adjuntado</p>}</div>
                </div>
              </div>
              <div>
                  ...
                  <div className="space-y-2">
                    {perfilParaVer.test_results.map((tr: any, i: number) => (
                      <div key={i} className="flex justify-between items-center bg-[#252525] p-3 rounded text-sm">
                        <span className="text-gray-300 flex items-center gap-2">🧠 {tr.test}</span>
