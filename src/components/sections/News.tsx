    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {news.length > 0 ? (
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scrollbar-hide">
            {news.map(item => (
              <div key={item.id} className="min-w-[280px] md:min-w-[350px] snap-start bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-5 shadow-lg flex flex-col">
                ...
                {item.url && (
                  <a href={item.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-[#E6CA65] hover:underline mb-4 break-all">🔗 {item.url}</a>
                )}
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  {!isAdmin && (
                    <a href={`https://wa.me/5492657234459?text=${encodeURIComponent("Hola, quisiera más información sobre: " + item.titulo)}`} target="_blank" rel="noopener noreferrer"
                      className="w-full bg-green-600 text-white font-bold py-2 rounded-lg text-sm hover:bg-green-700 flex items-center justify-center gap-2 transition">💬 Solicitar más información</a>
                  )}
                  {isAdmin && (
                    <div className="flex gap-2">
                      <button onClick={() => editarNovedad(item)} className="flex-1 bg-blue-900/30 text-blue-400 border border-blue-800/50 py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 hover:bg-blue-900/50">✏️ Editar</button>
                      <button onClick={() => eliminarNovedad(item.id)} className="flex-1 bg-red-900/30 text-red-400 border border-red-800/50 py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 hover:bg-red-900/50">🗑️ Eliminar</button>
                    </div>
                  )}
                </div>
              </div>
          ...
          <p className="text-gray-500 text-center py-8">No hay novedades.</p>
        )}
      </div>
    </div>
  );
